...
describe('TemporalBin tests', () => {
  ...
  it('test function makeUniformBins', () => {
    ...
    const min = moment('2016-02-08T15:00:00.000').startOf('M').toDate();
    const max = moment('2016-03-08T15:00:00.000').startOf('M').toDate();
    ...
    expect(res.length).equal(4);
    then = new Date("05/09/2016 14:20:30")
    res = temporalBin.makeUniformBins(now, then, 'M', 1)
  ...
  it('test function makeAutoBins', () => {
    let now = new Date("01/08/2016 15:00:00");
    let then = new Date("12/02/2016 14:20:30");
    let res = temporalBin.makeAutoBins(now, then, 11);
    expect(res.length).equal(12);
    res = temporalBin.makeAutoBins(now, then, 4)
    expect(res.length).equal(4);
  ...
  it('test function makeAutoBins with invalid input', () => {
    ...
    }).to.throw('max date must be greater than min date');
    expect(() => {
      // @ts-ignore
